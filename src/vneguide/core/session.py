...
from vneguide.ai import ExtractionOutcome, StructuredExtractor
...
class ConversationSession:
    ...
    def send(self, message: str) -> TurnResult:
        ...
        outcome = self._extractor.extract(message)
        ...
        if outcome.classification == "ambiguous" or outcome.procedure_code is None:
            question = outcome.clarification_question or "Bạn cần thực hiện thủ tục cụ thể nào?"
            return self._reply_without_draft_change(message, question, NextAction.ASK_CLARIFICATION)
        ...
        for field_id, value in outcome.fields.items():
            ...
            if field_id in draft.confirmed_fields and draft.values.get(field_id) == value:
                continue
            suggestions = [
                item
        ...
        if previous_missing and previous_missing[0] not in outcome.fields:
        ...
        return self._finish_turn(message, reply, action, extracted_fields=outcome.fields)

    def accept_suggestion(self, suggestion_id: str) -> TurnResult:
        return self._resolve_suggestion(suggestion_id, SuggestionStatus.ACCEPTED)
    ...
    def reject_suggestion(self, suggestion_id: str) -> TurnResult:
        ...
        self._state = replace(
            self._state,
            suggestions=self._with_status(suggestion_id, SuggestionStatus.REJECTED),
            clarification_attempts=attempts,
        )
    ...
    def edit_suggestion(self, suggestion_id: str, value: JSONValue) -> TurnResult:
        return self._resolve_suggestion(suggestion_id, SuggestionStatus.EDITED, edited_value=value)
    ...
    def _resolve_suggestion(
        ...
        *,
        ...
        if suggestion.revision != draft.revision:
            raise ValueError("Suggestion is stale for the current draft revision")
        ...
        self._rules.validate_field_value(code, suggestion.field_id, value)
        ...
        for item in self._state.suggestions:
            if item.suggestion_id == suggestion_id:
                ...
                updated.append(item)
        self._state = replace(self._state, draft=new_draft, suggestions=tuple(updated))
    ...
    def _with_status(
        ...
        return tuple(
            replace(item, status=status) if item.suggestion_id == suggestion_id else item
            for item in self._state.suggestions
        )
